    ...
    private void configureArguments() {
        ...
        switch (this.stepName) {
            ...
            case F5C_EVENT_ALIGNMENT:
                ...
                break;
            default:
                System.err.println("Cannot come here");
                break;
        }
    }

    private void minimap2Arguments() {
        Argument argument = new Argument(true, "reference index", "draft.fai", "Path to the reference index file", false,
                null, false);
        arguments.add(argument);
        argument = new Argument(true, "query sequence", "reads.fasta", "Path to the query sequence file", false, null, false);
        arguments.add(argument);
        argument = new Argument(true, "output file", "minimap2.sam", "Path to the output sam file", true, "-o", false);
        arguments.add(argument);
...
//        arguments.add(argument);
    ...
    private void samtoolSortArguments() {
        Argument argument = new Argument(true, "input sam", "minimap2.sam", "Path to the input sam file", false, null, false);
        arguments.add(argument);
        argument = new Argument(true, "output file", "samsort.bam", "Path to the output file", true, "-o", false);
        arguments.add(argument);
    ...
    private void samtoolIndexArguments() {
        Argument argument = new Argument(true, "input bam", "samsort.bam", "Path to the input bam file", false, null, false);
        arguments.add(argument);
        argument = new Argument(false, "threads", null, "Number of threads[1]", true, "-@", false);
        arguments.add(argument);
    }
